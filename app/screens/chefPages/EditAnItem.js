...
export default function EditAnItem({navigation, route}) {
    ...
    useEffect(() => {
        ...
        .then((json) => {
            ...
            if (camUri === undefined) {
                setUri("http://192.168.1.74:5000/" + json.product.productImage)
                console.log(uri)
            }
            if (camUri !== undefined) {
                setUri(camUri)
    ...
    return(
        <SafeAreaView style={styles.container}>
            <ScrollView showsVerticalScrollIndicator={false}>
                <Text style={{fontSize: 30, textAlign: 'center', paddingTop: 5, paddingBottom: 20}}>Edit An Item</Text>
                <View style={styles.photo}>
                    <Text style={{textAlign: 'center', fontSize: 20, fontWeight: 'bold', paddingBottom: 10}}>Photo</Text>
                    <Button type={'solid'} buttonStyle={{backgroundColor: 'blue', width: responsiveWidth(90), borderRadius: 10}} onPress={() => navigation.navigate('Camera Page', {from: "Edit An Item"})} title={'Take a New Picture'} />
                ...
                <View style={styles.discriptionView}>
                    ...
                    <TextInputNative numberOfLines={3} style={styles.discription} multiline={true} placeholder="Description Here..." value={description} onChangeText={text => setDescription(text)} />
                </View>
                ...
                <View style={styles.prefrenceView}>
                    ...
                    <View style={{paddingTop: 25}}>
                        <Button type={'solid'} buttonStyle={{backgroundColor: 'blue', width: responsiveWidth(90), borderRadius: 10}} title={"Edit Your Item"} onPress={async () => {
                            ...
                            formData.append('productImage', { uri: uri.replace("\\", "/"), name: Date.now() + uri.replace("\\", "/"), type: 'image/jpeg'})
                            fetch('http://192.168.1.74:5000/products/'+productId.toString(),{
                                method: 'PATCH',
                                headers: {
