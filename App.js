  ...
  return (
    <SafeAreaView
      ...
      <View>
        ...
        <Text
          style={{color: '#1abc9c', textAlign: 'center', top: 5}}
...
export default function FirstStack({navigation}) {
  ...
  const authContext = useMemo(
    () => ({
      signIn: async data => {
        fetch('http://192.168.1.73:5000/user/login', {
          method: 'POST',
          headers: {
      ...
      <AuthContext.Provider value={authContext}>
        <firstStartStack.Navigator>
          ...
          {state.userToken == null ? (
            <>
              <firstStartStack.Screen
                name="First Start Page"
                component={FirstStartScreen}
                options={{headerShown: false}}
              />
              <firstStartStack.Screen
                name="Log In Page"
          ...
          {state.userToken !== null && state.accountType === 'user' ? (
            <>
              <firstStartStack.Screen
                name="Home"
                component={userHomeDrawer}
                options={{headerShown: false}}
              />
              <firstStartStack.Screen
                name="Chef Profile Screen"
              ...
              <firstStartStack.Screen
                name="User Full Menu"
                component={userFullMenu}
                options={{headerShown: false}}
              />
              <firstStartStack.Screen
                name="User Manage Order"
                component={UserManageOrder}
                options={{headerShown: false}}
              />
            </>
          ) : (
          ...
          {state.userToken !== null && state.accountType === 'chef' ? (
            <>
              ...
              <firstStartStack.Screen
                name="Chef Item Page"
                component={ChefItemPage}
                options={{headerShown: false}}
              />
              <firstStartStack.Screen
                name="Edit An Item"
                component={EditAnItem}
                options={{headerShown: false}}
              />
              <firstStartStack.Screen
                name="Chef Full Menu"
                component={chefFullMenu}
                options={{headerShown: false}}
              />
              <firstStartStack.Screen
                name="Chef Manage Order"
                component={ChefManageOrder}
                options={{headerShown: false}}
              />
            </>
          ) : (
  ...
  return (
    <userbottomTab.Navigator>
      <userbottomTab.Screen
        options={{
          tabBarIcon: () => {
            return <Ionicons name="logo-xbox" color="black" size={30} />;
          },
        }}
        name="Delivery"
        component={HomePage}
      ...
      <userbottomTab.Screen
        options={{
          tabBarIcon: () => {
            return <Ionicons name="logo-tux" color="black" size={30} />;
          },
        }}
        name="Search"
...
function reportABug({navigation, route}) {
  ...
  return (
    <SafeAreaView style={styles.container}>
      <Text style={{fontSize: 32, position: 'absolute', top: 10}}>
        Report a Bug
      </Text>
      <Divider
        style={{
  ...
  function sendBug() {
    ...
    email(to, {
      body: discription.toString(),
      subject: 'New Make-e-Me Bug Report!',
    }).catch(console.error);
  }
}
